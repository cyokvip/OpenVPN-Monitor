...
class OpenvpnHtmlPrinter(object):
    ...
    def print_session_table_headers(vpn_mode):
        ...
        print('<table class="neirong table table-striped table-bordered table-hover ')
    ...
    def print_server_session(session):
        ...
        print('<td class="username_tag">{0!s}</td>'.format(session['username']))
        ...
        #print('<td class="jieshou_tag">{0!s} ({1!s})</td>'.format(bytes_recv, naturalsize(bytes_recv, binary=True)))
        print('<td class="jieshou_tag">{0!s}</td>'.format(bytes_recv, naturalsize(bytes_recv, binary=True)))
        print('<td class="fasong_tag">{0!s}</td>'.format(bytes_sent, naturalsize(bytes_sent, binary=True)))
        print('<td class="start_time_tag">{0!s}</td>'.format(
            session['connected_since'].strftime('%Y/%m/%d %H:%M:%S')))
        if 'last_seen' in session:
            print('<td>{0!s}</td>'.format(
                session['last_seen'].strftime('%Y/%m/%d %H:%M:%S')))
        else:
            print('<td>ERROR</td>')
        print('<td class="online_time_tag">{0!s}</td>'.format(total_time))
